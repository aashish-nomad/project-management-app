...
export default function ProjectsSidebar({ onProjectCreate, projects, onProjectSelect, selectedProjectId }) {
  return (
    <aside className="w-1/3 px-8 py-16 bg-stone-900 text-stone-50 md:w-72 rounded-r-xl">
      ...
      <ul className="mt-8">
        {projects.map(project => {
          ...
          return (
            <li key={project.id}>
              <button onClick={() => onProjectSelect(project.id)} className={cssClasses}>
                {project.title}
              </button>
            </li>
          );
